...
auto InsertExecutor::Next([[maybe_unused]] Tuple *tuple, RID *rid) -> bool {
    ...
    //std::cout<< "InsertExecutor::Next start tuple={" << tuple->ToString(&(GetExecutorContext()->GetCatalog()->GetTable(plan_->TableOid())->schema_)) 
    //            << "}, rid={" << *rid << "}" << std::endl;
    ...
    //std::cout<< "InsertExecutor::Next done tuple={" << tuple->ToString(&(GetExecutorContext()->GetCatalog()->GetTable(plan_->TableOid())->schema_)) 
    //            << "}, rid={" << *rid << "}" << std::endl;
...
void InsertExecutor::Insert(Tuple *tuple, RID *rid) {
    ...
    // 遍历 indexs, 用入参 tuple 构造K 插入到b+树中;  上面是插入了一行值, 但一行有多列, 改行的每个列吸入b+树中; 并事务记录操作
    // 事务操作肯定事务作用, 具体咋作用的咱也没具体分析; 插入b+树肯定是为了查询, 但查询怎么用的咱也没搞,
    ...
    for (auto it = indexes_.begin(); it != indexes_.end(); it++) {
        ...
        // 获取b+树, 插入b+树
        //index_oid - > b+树
        bpIndex = reinterpret_cast<BPlusTreeIndex<GenericKey<8>, RID, GenericComparator<8>> *>((*it)->index_.get());    // 模板使用, key 是一个8位的数字key; 获取b+树索引
